...
void Graph::addEdge(int u, int v, int weight) {
    // check if U is equal to V or if the edge UV already exists.
    if (edgeExist(u, v) || u == v){
        return;
    }
    // if u dont have edges from him, add new edge
    if (adjacencyList[u].isEmpty()){
        adjacencyList[u].add(Edge(u, v, weight));
    }
    else{
        int i = 0;
        // Loop over all the neighbors of U to find the exact place to add it to the list (according to vertex numbers)
    ...
    // if the new edge dont exixt yet add it
    if (!edgeExist(v , u)){
        addEdge(v , u, weight);
    }
}
//check if some edge are exist in the graph
...
void Graph::removeEdge(int u, int v) {
    // if u dont have neighbors throw exception
    if (adjacencyList[u].isEmpty()) {
        throw std::invalid_argument("the edge is not found");
    }
    int indexToRemove = -1;
    // Loop over all the neighbors of U to find the exact index of v
    for (int i = 0; i < adjacencyList[u].getNumOfElements(); i++){
        if(adjacencyList[u].get(i).getDestination() == v){
            indexToRemove = i;
            break;
    ...
    // if we dont find v throw exception
    if (indexToRemove == -1){
        throw std::invalid_argument("the edge is not found");
    }
    //remove the edge
